/**
 * Interactive Command-Line Runner for bwxBASIC
 * Executes an arbitrary BASIC program and then drops into a REPL loop.
 */
...
IO.print = (txt, newline = true, color = null) => {
    // We intercept IO.prompt to show the readline prompt instead of printing ']'
    if (txt === "]" && !newline) {
        if (rl) rl.prompt();
        return;
    }
...
// 5. Run the Engine & Start REPL
const run = async () => {
    const targetFile = process.argv[2];
    if (!targetFile) {
        console.error("Usage: node tests/run_repl.js <path_to_basic_file>");
        process.exit(1);
    }
    ...
    try {
        ...
        rl.on('line', (line) => {
            ...
            // Re-route the line buffer to bwxBASIC's command handler
            IO.handleCommand(input);

            // If the command hasn't naturally called IO.prompt(), we force it
            if (!SYS.running) {
                rl.prompt();
            }
        }).on('close', () => {
